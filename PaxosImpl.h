...
namespace volePSI {
    ...
    void PaxosHash<IdxType>::buildRow32(const block *hash, IdxType *row) const
    {
        if (mWeight == 3 /* && mSparseSize < std::numeric_limits<u32>::max()*/) {
            ...
            for (u64 i = 0; i < 2; ++i) {
                ...
                std::array<block *, 3> row128{ row128_[0] + i * 8, row128_[1] + i * 8, row128_[2] + i * 8 };
    ...
    template <typename IdxType>
    void PaxosHash<IdxType>::hashBuildRow32(const block *inIter, IdxType *rows, block *hash) const
    {
    ...
    template <typename IdxType>
    void PaxosHash<IdxType>::hashBuildRow1(const block *inIter, IdxType *rows, block *hash) const
    {
    ...
    {
        auto size =
            sizeof(IdxType) * (mNumItems * mWeight) + sizeof(span<IdxType>) * mSparseSize + sizeof(IdxType) * (mNumItems * mWeight) + sizeof(block) * mNumItems;
    ...
    void Paxos<IdxType>::init(u64 numItems, PaxosParam p, block seed)
    {
        if (p.mSparseSize >= u64(std::numeric_limits<IdxType>::max())) {
            std::cout << "the size of the paxos is too large for the index type. " << p.mSparseSize << " vs " << u64(std::numeric_limits<IdxType>::max()) << " "
                      << LOCATION << std::endl;
            throw RTE_LOC;
        }
    ...
    void Paxos<IdxType>::decode(span<const block> inputs, MatrixView<ValueType> values, MatrixView<const ValueType> p)
    {
        ...
        } else if (values.cols() * sizeof(ValueType) % sizeof(block) == 0 && std::is_same<ValueType, block>::value == false) {
            // reduce ValueType to block if possible.

            auto n = values.rows();
            auto m = values.cols() * sizeof(ValueType) / sizeof(block);

            decode<block>(inputs, MatrixView<block>((block *)values.data(), n, m), MatrixView<const block>((block *)p.data(), p.rows(), m));
        } else {
    ...
    template <typename IdxType>
    void Paxos<IdxType>::setInput(MatrixView<IdxType> rows, span<block> dense, span<span<IdxType>> cols, span<IdxType> colBacking, span<IdxType> colWeights)
    {
        if (rows.rows() != mNumItems || dense.size() != mNumItems)
            throw RTE_LOC;
        if (rows.cols() != mWeight)
    ...
    std::pair<PaxosPermutation<IdxType>, u64> Paxos<IdxType>::computePermutation(
        span<IdxType> mainRows, span<IdxType> mainCols, span<std::array<IdxType, 2>> gapRows, bool withDense)
    {
        ...
        for (u64 i = 0; i < cols; ++i) {
            if (cColSet.find(i) == cColSet.end() && gapColSet.find(i) == gapColSet.end()) {
                colPerm.push_back(i);
            }
        }
    ...
    template <typename IdxType>
    void Paxos<IdxType>::triangulate(std::vector<IdxType> &mainRows, std::vector<IdxType> &mainCols, std::vector<std::array<IdxType, 2>> &gapRows)
    {
                ...
                if (rowSet[rowIdx] == 0) {
                    ...
                    if (!!(first)) {
                        ...
                        if (mDense[mainRows.back()] == mDense[rowIdx]) {
                            std::cout << "Paxos error, Duplicate keys were detected at idx " << mainRows.back() << " " << rowIdx
    ...
    template <typename IdxType>
    template <typename Vec, typename ConstVec, typename Helper>
    Vec Paxos<IdxType>::getX2Prime(FCInv &fcinv, span<std::array<IdxType, 2>> gapRows, span<u64> gapCols, const ConstVec &X, const Vec &P, Helper &helper)
    {
    ...
    template <typename IdxType>
    oc::DenseMtx Paxos<IdxType>::getEPrime(FCInv &fcinv, span<std::array<IdxType, 2>> gapRows, span<u64> gapCols)
    {
        auto g = gapRows.size();
    ...
    void Paxos<IdxType>::backfill(
        span<IdxType> mainRows, span<IdxType> mainCols, span<std::array<IdxType, 2>> gapRows, ConstVec &X, Vec &P, Helper &h, PRNG *prng)
    {
    ...
    void Paxos<IdxType>::backfillBinary(
        span<IdxType> mainRows, span<IdxType> mainCols, span<std::array<IdxType, 2>> gapRows, ConstVec &X, Vec &P, Helper &h, oc::PRNG *prng)
    {
        auto g = gapRows.size();
    ...
    void Paxos<IdxType>::backfillGf128(
        span<IdxType> mainRows, span<IdxType> mainCols, span<std::array<IdxType, 2>> gapRows, ConstVec &X, Vec &P, Helper &helper, PRNG *prng)
    {
...
#define GF128_DENSE_BACKFILL                   \
    ...
    void Paxos<IdxType>::decode32(const IdxType *rows_, const block *dense_, ValueType *values_, Vec &p_, Helper &h)
    {
        //{
        //	auto r = rows_;
        //	auto d = dense_;
    ...
    template <typename IdxType>
    template <typename ValueType, typename Helper, typename Vec>
    void Paxos<IdxType>::decode8(const IdxType *rows_, const block *dense_, ValueType *values_, Vec &p_, Helper &h)
    {
    ...
    template <typename IdxType>
    template <typename ValueType, typename Helper, typename Vec>
    void Paxos<IdxType>::decode1(const IdxType *rows, const block *dense, ValueType *values, Vec &p, Helper &h)
    {
    ...
    template <typename IdxType>
    typename Paxos<IdxType>::FCInv Paxos<IdxType>::getFCInv(span<IdxType> mainRows, span<IdxType> mainCols, span<std::array<IdxType, 2>> gapRows) const
    {
        ...
        for (u64 i = 0; i < gapRows.size(); ++i) {
            if (std::memcmp(mRows[gapRows[i][0]].data(), mRows[gapRows[i][1]].data(), mWeight * sizeof(IdxType)) == 0) {
    ...
    template <typename IdxType>
    std::vector<u64> Paxos<IdxType>::getGapCols(FCInv &fcinv, span<std::array<IdxType, 2>> gapRows) const
    {
        if (gapRows.size() == 0)
            return {};
    ...
    void Baxos::solve(span<const block> inputs, MatrixView<const ValueType> values, MatrixView<ValueType> output, PRNG *prng, u64 numThreads)
    {
        ...
        if (values.cols() == 1) {
            ...
            solve<block>(
                inputs, MatrixView<const block>((block *)values.data(), n, m), MatrixView<block>((block *)output.data(), output.rows(), m), prng, numThreads);
        } else {
    ...
    template <typename Vec, typename ConstVec, typename Helper>
    void Baxos::solve(span<const block> inputs, ConstVec &V, Vec &P, PRNG *prng, u64 numThreads, Helper &h)
    {
    ...
    void Baxos::implParSolve(span<const block> inputs_, ConstVec &vals_, Vec &p_, PRNG *prng, u64 numThreads, Helper &h)
    {
#ifndef NDEBUG
        {
            std::unordered_set<block> inputSet;
        ...
        auto routine = [&](u64 thrdIdx) {
            ...
            auto allocSize = sizeof(IdxType) * (mItemsPerBin * mWeight * 2 + mPaxosParam.mSparseSize) + sizeof(span<IdxType>) * mPaxosParam.mSparseSize /*+
    ...
    void Baxos::computeRetrivalIdx(span<const block> inputs, std::vector<ValueType> &values, std::vector<std::vector<u64>> &idx, u64 numThreads)
    {
        ...
        computeIdx(inputs, values, idx, h, numThreads);
    }
        ...
        std::vector<block> &hashs, std::vector<std::vector<u64>> &rows, std::vector<block> &values, std::vector<std::map<u64, block>> &pp, u64 numThreads)
    {
        ...
        decodeIdx(hashs, rows, values, pp, h, numThreads);
    }
    ...
    template <typename Vec, typename ConstVec, typename Helper>
    void Baxos::decode(span<const block> inputs, Vec &V, ConstVec &P, Helper &h, u64 numThreads)
    {
        auto bitLength = oc::roundUpTo(oc::log2ceil((u64)(mPaxosParam.mSparseSize + 1)), 8);
        if (bitLength <= 8)
    ...
    void Baxos::computeIdx(span<const block> inputs, std::vector<block> &V, std::vector<std::vector<u64>> &P, Helper &h, u64 numThreads)
    {
        auto bitLength = oc::roundUpTo(oc::log2ceil((u64)(mPaxosParam.mSparseSize + 1)), 8);
        if (bitLength <= 8)
            implParComputeIdx<u8>(inputs, V, P, h, numThreads);
        else if (bitLength <= 16)
            implParComputeIdx<u16>(inputs, V, P, h, numThreads);
        else if (bitLength <= 32)
            implParComputeIdx<u32>(inputs, V, P, h, numThreads);
        else
            implParComputeIdx<u64>(inputs, V, P, h, numThreads);
    }

    template <typename Helper>
    void Baxos::decodeIdx(
        std::vector<block> &hashs,
        std::vector<std::vector<u64>> &rows,
        std::vector<block> &values,
        std::vector<std::map<u64, block>> &pp,
        Helper &h,
        u64 numThreads)
    {
        auto bitLength = oc::roundUpTo(oc::log2ceil((u64)(mPaxosParam.mSparseSize + 1)), 8);
        if (bitLength <= 8)
            implParDecodeIdx<u8>(hashs, rows, values, pp, h, numThreads);
        else if (bitLength <= 16)
            implParDecodeIdx<u16>(hashs, rows, values, pp, h, numThreads);
        else if (bitLength <= 32)
            implParDecodeIdx<u32>(hashs, rows, values, pp, h, numThreads);
        else
            implParDecodeIdx<u64>(hashs, rows, values, pp, h, numThreads);
    }

    template <typename IdxType, typename Vec, typename ConstVec, typename Helper>
    void Baxos::implDecodeBin(u64 binIdx, span<block> hashes, Vec &values, Vec &valuesBuff, span<u64> inIdxs, ConstVec &PP, Helper &h, Paxos<IdxType> &paxos)
    {
        ...
        std::array<IdxType, maxWeightSize> sparse_row_idx;
        ...
        for (u64 i = 0; i < hashes.size(); ++i) {
            paxos.mHasher.buildRow(hashes[i], sparse_row_idx.data());
            ...
            // } else
            paxos.decode1(sparse_row_idx.data(), &hashes[i], v, PP, h);
        }
    }
    ...
    void Baxos::implComputeIdxBatch(span<const block> inputs, std::vector<block> &hashs, std::vector<std::vector<u64>> &rows, Helper &h)
    {
        u64 decodeSize = std::min<u64>(512, inputs.size());
        Matrix<block> batches(mNumBins, decodeSize);
        Matrix<u64> inIdxs(mNumBins, decodeSize);
        ...
        std::array<IdxType, 20> sparse_row_idx;

        for (u64 i = 0; i < inputs.size(); ++i, ++inIter) {
            auto buffer = hasher.hashBlock(*inIter);

            // auto binIdx = buffer[k].as<u64>()[0] % mNumBins;
            auto binIdx = modNumBins(buffer);

            hashs[i] = buffer;

            paxos.mHasher.buildRow(buffer, sparse_row_idx.data());

            for (u64 j = 0; j < mWeight; ++j) {
                rows[i][j] = sparse_row_idx[j];
            }
            rows[i][mWeight] = binIdx;
        }
    }

    template <typename IdxType, typename Helper>
    void Baxos::implDecodeIdxBatch(
        std::vector<block> &hashs, std::vector<std::vector<u64>> &rows, std::vector<block> &values, std::vector<std::map<u64, block>> &pp, Helper &h)
    {
        u64 decodeSize = std::min<u64>(512, hashs.size());
        Matrix<block> batches(mNumBins, decodeSize);
        Matrix<u64> inIdxs(mNumBins, decodeSize);
        std::vector<u64> batchSizes(mNumBins);
        ...
        for (u64 i = 0; i < hashs.size(); ++i) {
            u64 binIdx = rows[i][mWeight];

            h.assign(values.data() + i, &pp[binIdx][rows[i][0]]);
            for (u64 j = 1; j < mWeight; ++j) {
                h.add(values.data() + i, &pp[binIdx][rows[i][j]]);
            }

            block x = hashs[i];
            h.multAdd(values.data() + i, &pp[binIdx][paxos.mSparseSize], x);

            for (u64 j = 1; j < paxos.mDenseSize; ++j) {
                x = x.gf128Mul(hashs[i]);
                h.multAdd(values.data() + i, &pp[binIdx][j + paxos.mSparseSize], x);
                // values[0] = values[0] ^ x.gf128Mul(p2[i + mSparseSize]);
            }
        }
    }
    ...
    template <typename IdxType, typename Vec, typename ConstVec, typename Helper>
    void Baxos::implParDecode(span<const block> inputs, Vec &values, ConstVec &pp, Helper &h, u64 numThreads)
    {
        if (mNumBins == 1) {
            Paxos<IdxType> paxos;
            paxos.init(1, mPaxosParam, mSeed);
    ...
    template <typename IdxType, typename Helper>
    void Baxos::implParComputeIdx(span<const block> inputs, std::vector<block> &values, std::vector<std::vector<u64>> &pp, Helper &h, u64 numThreads)
    {
        // if (mNumBins == 1) {
        //     Paxos<IdxType> paxos;
        //     paxos.init(1, mPaxosParam, mSeed);
        ...
        auto routine = [&](u64 i) {
            ...
            auto end = (inputs.size() * (i + 1)) / numThreads;
            span<const block> in(inputs.begin() + begin, inputs.begin() + end);
            implComputeIdxBatch<IdxType>(in, values, pp, h);
        };
        ...
        u64 numThreads)
    {
        ...
        std::vector<std::thread> thrds(numThreads - 1);
        auto routine = [&](u64 i) {
            auto begin = (hashs.size() * i) / numThreads;
            auto end = (hashs.size() * (i + 1)) / numThreads;
            std::vector<block> hashs_seg(hashs.begin() + begin, hashs.begin() + end);

            implDecodeIdxBatch<IdxType>(hashs_seg, rows, values, pp, h);
        };
