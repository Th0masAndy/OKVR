...
namespace volePSI {
    ...
    public:
        ...
        void encode(MatrixView<const ValueType> values, MatrixView<ValueType> output, oc::PRNG *prng = nullptr)
        {
            ...
            if (values.cols() == 1) {
                ...
                encode<block>(MatrixView<const block>((block *)values.data(), n, m), MatrixView<block>((block *)output.data(), n, m), prng);
            } else {
        ...
        // encoding.
        void setInput(MatrixView<IdxType> rows, span<block> dense, span<span<IdxType>> cols, span<IdxType> colBacking, span<IdxType> colWeights);
        ...
        void triangulate(std::vector<IdxType> &mainRows, std::vector<IdxType> &mainCols, std::vector<std::array<IdxType, 2>> &gapRows);

        // once triangulated, this is used to assign values
        // to output (paxos).
        template <typename Vec, typename ConstVec, typename Helper>
        void backfill(
            span<IdxType> mainRows, span<IdxType> mainCols, span<std::array<IdxType, 2>> gapRows, ConstVec &values, Vec &output, Helper &h, oc::PRNG *prng);
        ...
        FCInv getFCInv(span<IdxType> mainRows, span<IdxType> mainCols, span<std::array<IdxType, 2>> gapRows) const;

        // returns which columns are used for the gap. This
        // is only used for binary dense method.
        std::vector<u64> getGapCols(FCInv &fcinv, span<std::array<IdxType, 2>> gapRows) const;

        // returns x2' = x2 - D' r - FC^-1 x1
        template <typename Vec, typename ConstVec, typename Helper>
        Vec getX2Prime(FCInv &fcinv, span<std::array<IdxType, 2>> gapRows, span<u64> gapCols, const ConstVec &X, const Vec &P, Helper &h);

        // returns E' = -FC^-1B + E
        oc::DenseMtx getEPrime(FCInv &fcinv, span<std::array<IdxType, 2>> gapRows, span<u64> gapCols);
    ...
    public:
        ...
        template <typename ValueType>
        void solve(span<const block> inputs, span<const ValueType> values, span<ValueType> output, oc::PRNG *prng = nullptr, u64 numThreads = 0);
        ...
        // prng should be non-null if randomized paxos is desired.
        template <typename ValueType>
        void solve(span<const block> inputs, MatrixView<const ValueType> values, MatrixView<ValueType> output, oc::PRNG *prng = nullptr, u64 numThreads = 0);

        // solve/encode the system.
        template <typename Vec, typename ConstVec, typename Helper>
        void solve(span<const block> inputs, ConstVec &values, Vec &output, oc::PRNG *prng, u64 numThreads, Helper &h);
        ...
        template <typename Vec, typename ConstVec, typename Helper>
        void decode(span<const block> inputs, Vec &values, ConstVec &p, Helper &h, u64 numThreads);

        template <typename Helper>
        void computeIdx(span<const block> inputs, std::vector<block> &V, std::vector<std::vector<u64>> &P, Helper &h, u64 numThreads);
        ...
        void implParSolve(span<const block> inputs, ConstVec &values, Vec &output, oc::PRNG *prng, u64 numThreads, Helper &h);

        // create the desired number of threads and split up the work.
        template <typename IdxType, typename Vec, typename ConstVec, typename Helper>
        void implParDecode(span<const block> inputs, Vec &values, ConstVec &p, Helper &h, u64 numThreads);

        // create the desired number of threads and split up the work.
        template <typename IdxType, typename Helper>
        void implParDecodeIdx(
            std::vector<block> &hashs,
            std::vector<std::vector<u64>> &rows,
            std::vector<block> &values,
            std::vector<std::map<u64, block>> &pp,
        ...
        template <typename IdxType, typename Helper>
        void implComputeIdxBatch(span<const block> inputs, std::vector<block> &hashs, std::vector<std::vector<u64>> &rows, Helper &h);
